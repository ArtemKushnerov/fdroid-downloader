...
class InstrumentationHandler:
    ...
    def add_instrumentation_files(self):
        logging.info(f'{self.project.name}: INSTRUMENT')
        self.add_instrumentation_class()
        self.add_properties_file()

    def add_instrumentation_class(self):
        instrumentation_package = f'{self.project.app_path}/src/main/java/com/zhauniarovich/bbtester'
    ...
    def add_properties_file(self):
        jacoco_agent_properties_dir = f'{self.project.app_path}/src/main/resources/'
        ...
        with open(f'{jacoco_agent_properties_dir}/jacoco-agent.properties', 'w') as agent_file:
            agent_file.write(f'destfile=/mnt/sdcard/{self.project.name}/coverage.exec')
