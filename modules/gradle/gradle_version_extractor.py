...
class GradleVersionExtractor:
    ...
    def extract_gradle_version(self):
        logging.info(f'{self.project.name}: EXTRACT VERSION')
        build_gradle_path = os.path.join(self.project.path, 'build.gradle')
        ...
        if not gradle_dependency:
            app_build_gradle_path = os.path.join(self.project.app_path, 'build.gradle')
            ...
            if not gradle_dependency:
                raise AbsentGradleVersionException(self.project.name)

        version = gradle_dependency.group(1)
        version = self.cut_plus_notation(version)
        logging.info(f'{self.project.name}: VERSION={version}')
        return version
